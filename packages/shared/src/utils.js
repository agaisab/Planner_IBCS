...
export const computeWorkKindFor = (task, date, shifts) => {
  ...
  if (isWeekend(date)) return 'Nadgodziny 100%';
  const NIGHT = 22 * 60;
  const span = getPlanSpanMins(shifts);
  const startMinutes = toMinutes(task.start);
  const endMinutes = toMinutes(task.end);
  if (span.start != null && span.end != null && startMinutes >= span.start && endMinutes <= span.end) {
    return 'Zwykłe';
  }
  if (startMinutes >= NIGHT || endMinutes > NIGHT) return 'Nocne';
  return 'Nadgodziny 50%';
};
